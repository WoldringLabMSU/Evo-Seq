...
# Define the parse_with_retries function for handling XML parsing with retries
def parse_with_retries(xml_file, max_attempts=3):
    for attempt in range(max_attempts):
        try:
            return ET.parse(xml_file)  # Attempt to parse
        except ET.ParseError as e:
            print(f"Parse error in {xml_file}: {e}. Retrying {attempt + 1}/{max_attempts}...")
            time.sleep(10)  # Wait before retrying
    raise RuntimeError(f"Failed to parse {xml_file} after {max_attempts} attempts.")

# Interact with BlastP, and record the XML
def NCBI_to_XML(dirname, sequence, hits=2000, expect_value=0.30, seq_name='', attempts=3):
    ...
    for attempt in range(attempts):
        try:
            # Run the NCBI BLAST search and get results
            print(f"Attempt {attempt + 1}/{attempts} to retrieve BLAST results for {seq_name}")
            ...
            xml_path = f"./{dirname}/{XML_Name}"
            with open(xml_path, "w+") as fout:
                fout.write(blast_result_handle.read())
            print(f"Successfully retrieved and wrote BLAST results to {xml_path}")

            # Parse XML with retry handling
            print(f"Parsing XML file: {xml_path}")
            blastp_xml = parse_with_retries(xml_path)
            print(f"Successfully parsed {xml_path}")
            return blastp_xml # Returns the ElementTree type from the xml library

        except URLError as e:
            print(f"Error fetching BlastP results: {e}. Retrying {attempt + 1}/{attempts}...")
            time.sleep(300)  # Wait a minute before retrying
    
    # If all attempts fail, raise an error
    raise RuntimeError("There was an error fetching the BlastP results after multiple attempts.")
...
def Parse_BlastP_XML(dirname, blastp_xml, sequence, sequence_name=None):
    ...
    # If no sequence_name is identified (Meaning this is the search with the user sequence)
    if sequence_name is None:
        if isinstance(sequence, str):
            with open(f"./{dirname}/HitsInfo.csv", "a+") as fout:
                fout.write(f"Hit ID,Hit Description,Hit Sequence,Species Name\n")  # Added Species Name column
                ...
                for hit in blastp_xml.findall(
                    ...
                    # Extract species name from Hit_def (assuming it's in square brackets [])
                    species_name_match = re.search(r'\[(.*?)\]', hitdef)
                    species_name = species_name_match.group(1) if species_name_match else "Unknown_Species"
                    # Combine accession number and species name
                    new_hitid = f"{hitaccession} ({species_name})"
                    # If the sequence doesn't have unknown amino acids (X)
                    if ("X" not in seq):
                        fout.write(f"{new_hitid},{hitdef},{seq},{species_name}\n")
                        Fasta_Dict[new_hitid] = seq  # Store new ID with species name
            # Write the Fasta_Dict to a FASTA file
        ...
        # If we've been given multiple sequences, we have a dict for sequence and a list of xmls for blastp_xml
            ...
            for xml in blastp_xml:
                with open(f"./{dirname}/HitsInfo.csv", "a+") as fout:
                    ...
                    for hit in xml.findall(
                            './BlastOutput_iterations/Iteration/Iteration_hits/Hit'):
                        # I've tried to also add the Hit_accession, but I can't access that form the XML for some reason
                        hitid = (hit.find('Hit_id')).text
                        hitdef = (hit.find('Hit_def')).text
                        hitaccession = (
                            hit.find('Hit_accession')).text.replace(
                        ...
                        # If the sequence doesn't have unknown amino acids
                        if ("X" not in seq):
                            fout.write(f"{new_hitid},{hitdef},{seq},{species_name}\n")
                            Fasta_Dict[new_hitid] = seq  # Store with combined key

                # Ensure all user sequences are in the final FASTA file
        ...
        return (return_name)
    # Handling supplemental searches with sequence name
...
def BlastP(dirname, sequence, hits=1000, expect_value=0.2, sequence_name=None):
...
def CDHit(dirname, fin, fout, cutoff, supplement=False):
    try:
        # Run CD-Hit with user parameters and allocate 2GB of memory
        os.system(
            f'{CDHIT_Executable} -i {dirname}/{fin} -o {dirname}/{fout} -c {cutoff} -n {CDHit_Cutoff(cutoff)} -M 2048')
    except BaseException:
...
def IQTree_Phylo(dirname, finname):
    if args.tree_method == 'iqtree':
        # Original IQ-TREE commands
        if not os.path.isdir(f"{dirname}/IQTree_Phylo"):  # Make the directory
            os.mkdir(f"{dirname}/IQTree_Phylo")
        try:
            os.system(f'{IQTREE_Executable} -s {dirname}/{finname} -pre {dirname}/IQTree_Phylo/Phylo -alrt 20000 -bb 20000 -nt AUTO')
        except BaseException:
            print("There was an error building the phylogeny with IQ-TREE.")
            raise RuntimeError("There was an error building the phylogeny with IQ-TREE.")
    elif args.tree_method == 'baliphy':
        # Run BaLiPhy
        output_dir = os.path.join(dirname, "BaLiPhy_Phylo")
        run_baliphy(finname, output_dir)
        
# Define BaLiPhy tree construction function
def run_baliphy(input_fasta, output_dir):
    """
    Runs BaLiPhy to construct phylogeny, alignment, and ancestral sequence data.
    """
    # Ensure output directory exists
    os.makedirs(output_dir, exist_ok=True)
    
    # Command to run BaLiPhy with the specified parameters
    baliphy_cmd = (
        f"singularity exec /mnt/home/k0099424/bali-phy-4.0-beta8-linux64-intel-singularity.sif "
        f"bali-phy {input_fasta} --alphabet Amino-Acids --smodel 'lg08 +> Rates.gamma' --imodel RS07 "
        f"--output {output_dir}"
    )

    # Execute the BaLiPhy command
    try:
        subprocess.run(baliphy_cmd, shell=True, check=True)
        print("BaLiPhy run completed successfully.")
    except subprocess.CalledProcessError as e:
        print("Error running BaLiPhy:", e)
        raise
# Implement BaLiPhy output parsing functions
def parse_baliphy_output(output_dir):
    """
    Parse BaLiPhy output files to retrieve the alignment, tree, and ancestral sequences.
    """
    # Example of parsing alignment file
    alignment_file = os.path.join(output_dir, "alignment.fasta")  # Update based on actual output files
    with open(alignment_file) as af:
        alignment_data = af.read()

    # Parse tree file
    tree_file = os.path.join(output_dir, "Phylo.tree")  # Adjust to actual BaLiPhy tree file
    with open(tree_file) as tf:
        tree_data = tf.read()

    # Parse ancestral sequences (example, adjust filename)
    ancestor_file = os.path.join(output_dir, "ancestor_sequences.fasta")  # Adjust if different
    with open(ancestor_file) as ancf:
        ancestor_data = ancf.read()

    return alignment_data, tree_data, ancestor_data

def extract_descendant_info(tree_data, ancestor_id):
    """
    Identify descendants of a given ancestor node using the consensus tree.
    """
    # (Placeholder code; replace with actual extraction code)
    descendants = []
    # Logic to parse tree_data and populate descendants
    return descendants

def map_ancestors_to_descendants(consensus_tree, ancestor_nodes):
    """
    Map each ancestor to its descendant set on the consensus tree.
    """
    ancestor_mapping = {}
    for ancestor in ancestor_nodes:
        descendant_set = extract_descendant_info(consensus_tree, ancestor)
        ancestor_mapping[ancestor] = descendant_set
    return ancestor_mapping

def IQTree_ASR(dirname, finname):
    if args.tree_method == 'iqtree':
        # Original IQ-TREE ASR logic
        model = ''
        try:
        ...
        try:
            ...
            print("There was an error conducting ASR with IQ-TREE.")
            raise RuntimeError("There was an error conducting ASR with IQ-TREE.")
    elif args.tree_method == 'baliphy':
        # Parse BaLiPhy output for alignment, tree, and ancestor sequences
        output_dir = os.path.join(dirname, "BaLiPhy_Phylo")
        alignment_data, tree_data, ancestor_data = parse_baliphy_output(output_dir)
        print("Parsed BaLiPhy alignment, tree, and ancestral sequences successfully.")
...
def Binary_Gap_Analysis(dirname, finname):
    ...
    try:
        ...
        if args.tree_method == 'iqtree':
            ...
            if not os.path.isdir(f"{dirname}/BaLiPhy_Binary"):  # Create BaLiPhy-specific directory for binary gap analysis
                os.mkdir(f"{dirname}/BaLiPhy_Binary")
            tree_file = f'{dirname}/BaLiPhy_Phylo/Phylo.tree'  # Corrected tree file path
            if not os.path.exists(tree_file):
                raise FileNotFoundError(f"BaLiPhy tree file not found at {tree_file}")
            os.system(f'{BALIPHY_Executable} --input {dirname}/IQTree_Binary/Binary_Alignment.fasta --tree {tree_file} --output {dirname}/BaLiPhy_Binary/Binary')
    except BaseException:
...
if __name__ == '__main__':
    ...
    # Adding subparsers for multiple 'modes' of operation
    ...
    # ASR mode for conducting ASR and building libraries
    parser_mode2 = subparsers.add_parser(
        'ASR', help='Conduct ASR and generate protein libraries.')
    
    # Tree method choice with a default set to 'baliphy'
    parser_mode2.add_argument(
        '--tree_method',
        choices=['baliphy', 'iqtree'],
        default='baliphy',
        help="Choose the method for tree inference and ASR: 'baliphy' (default) or 'iqtree'.")

    # Arguments specific to ASR mode
    parser_mode2.add_argument(
        '-n', '--name',
        action='store',
        dest='directory',
    ...
    parser_mode2.add_argument(
        ...
        type=float,
        help='Supplementation is needed when dataset diversity must be improved. We recommend beginning with a value of 75%.')
    parser_mode2.add_argument(
        "-o", "--outgroup",
        action="store",
        dest="outgroup",
        help="Optional: outgroup sequences input as the full FASTA file name")
    parser_mode2.add_argument(
        '--cdhit',
        action='store',
        dest='cdhitexe',
    ...
    parser_mode2.add_argument(
        ...
        dest='baliphyexe',
        default='bali-phy',
        help='The executable for BaLiPhy (default bali-phy)')
    parser_mode2.add_argument(
        '--MSI',
        action='store',
    ...
    # MakeFigures mode for creating figures from prior ASR data
    ...
    # Adding the tree argument to ASR mode to specify the path to the tree file
    parser_mode2.add_argument(
        '--tree',
        action='store',
        dest='treefile',
        required=True,
        help='The path to the tree file generated by BaLiPhy or IQ-TREE.')    

    # RemakeLibraries mode for generating new libraries with a new cutoff
    ...
    parser_mode4.add_argument(
        ...
        action="store_true",
        dest="Alt_All",
        help='Turns on Alt-All library mode as described by https://doi.org/10.1093/molbev/msw223')
    
    # Parse the arguments
    args = parser.parse_args()

    # Set the directory based on the mode
    ...
    if args.mode == 'ASR':
        ...
        BALIPHY_Executable = args.baliphyexe
        ...
        if Final_Dataset_Size < 50:
            print("Final dataset size too small. Setting to minimum of 50 sequences.")
            Final_Dataset_Size = 50

        if args.supplementationCutoff is not None:
            supplement_Cutoff = args.supplementationCutoff / 100 if (100 > args.supplementationCutoff > 40) else args.supplementationCutoff
            if not (0.9 > supplement_Cutoff > 0.4) or supplement_Cutoff == 0:
                raise ValueError("Supplementation cutoff must be between 90% and 40%.")
        else:
            supplement_Cutoff = 0

    # Check if Final Sequences file already exists
    if os.path.exists(f"{directory}/Final_Sequences.fasta"):
        print("Detected previously processed dataset. Proceeding with selected ASR method.")
        Final_Name = "Final_Sequences.fasta"
    else:
        # Process user input sequence if not pre-processed
        if '.' not in args.input:
            sequence = args.input.upper().replace("-", '')
            if any(n not in AA_key for n in sequence):
                raise ValueError("Invalid amino acids in sequence. Please verify input.")
            Blastp_out_name = BlastP(directory, sequence)
            Final_Name = Sequence_Processing(directory, Blastp_out_name, sequence, args.outgroup)
        else:
            if not os.path.exists(args.input):
                raise ValueError("Input FASTA file not found.")
            User_Input_Sequence = fasta2dict(args.input)
            ...
            Final_Name = Sequence_Processing(directory, Blastp_out_name, User_Input_Sequence, args.outgroup)

    with open(f"{directory}/{Final_Name}") as fin:
        if len(fin.readlines()) < 80:
            print("WARNING: Few sequences available; this may affect ASR reliability.")

    # Run the chosen ASR and tree inference method
    if args.tree_method == 'baliphy':
        ASR_Statefile_Dict = run_baliphy(directory, Final_Name)
    elif args.tree_method == 'iqtree':
        IQTree_Phylo(directory, Final_Name)
        ASR_Statefile_Dict = IQTree_ASR(directory, Final_Name)

    # Only run Binary_Gap_Analysis if IQ-TREE is used
    if args.tree_method == 'iqtree':
        Binary_Statefile_Dict = Binary_Gap_Analysis(directory, Final_Name)
    else:
        Binary_Statefile_Dict = None  # Set to None or skip if BaLiPhy

    # Write confidences and generate uncertainty libraries only if Binary_Statefile_Dict is available
    if Binary_Statefile_Dict is not None:
        ...
        # Handle BaLiPhy-specific library generation if needed, or skip these steps
        print("Skipping Binary Gap Analysis for BaLiPhy")
